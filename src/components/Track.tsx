...
export default function Track({ index, track }: { index: number; track: AudioTrack }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 px-2">
      ...
      <Led color="red-500" />
      <Slider value={level} onChange={(e) =>
          handleGainChange(Array.isArray(e.value) ? e.value[0] : e.value)} orientation="vertical" style={{width: '1.5rem'}} />
      ...
      {showSettings && (
        <TrackSettingsModal track={track} onClose={() => setShowSettings(false)} />
      )}
    </div>
  );
}
